"""
test_kappa.py â€” pytest test suite for ConfusionMapper core functions

Tests cover:
  - compute_cohens_kappa()   : Îº formula, edge cases, interpretation thresholds
...
# ---------------------------------------------------------------------------
# Import path setup â€” works whether tests/ is run from repo root or directly
# ---------------------------------------------------------------------------
...
# ===========================================================================
# SECTION 1 â€” compute_cohens_kappa()
# ===========================================================================

class TestCohenKappaPerfectAgreement:
    """Îº = 1.0 when both raters agree on every item."""
    ...
    def test_perfect_agreement_all_same_label(self):
        ...
        Worked example:
          human = ai = ["CF", "CF", "CF"]
          Po = 3/3 = 1.0
          Pe = (3/3 * 3/3) = 1.0  â†’ guarded case â†’ Îº = 1.0
        """
        human = ["CF", "CF", "CF"]
        ai    = ["CF", "CF", "CF"]
    ...
    def test_perfect_agreement_mixed_labels(self):
        ...
        Worked example:
          ...
          Po = 4/4 = 1.0
          Pe = (1/4)(1/4) Ã— 4 categories = 4/16 = 0.25
          Îº = (1.0 âˆ’ 0.25) / (1.0 âˆ’ 0.25) = 0.75 / 0.75 = 1.0
        """
        human = ["RF", "PK", "CF", "INT"]
        ai    = ["RF", "PK", "CF", "INT"]
        result = compute_cohens_kappa(human, ai)
...
class TestCohenKappaKnownValues:
    """
    Îº computed against manual calculation for a specific input.
    ...
    def test_known_kappa_substantial(self):
        ...
        Worked example (verified by hand):
          ...
          Agreements (position-by-position):
            CF==CF âœ“, CF==CF âœ“, CF==CF âœ“, CFâ‰ RF âœ—,
            RF==RF âœ“, RF==RF âœ“, PK==PK âœ“, PKâ‰ CF âœ—,
            INT==INT âœ“, INT==INT âœ“
          ...
          Îº = (0.8 âˆ’ 0.28) / (1.0 âˆ’ 0.28)
            = 0.52 / 0.72
            = 0.7222...
            â‰ˆ 0.7222 (rounded to 4 d.p.)
        """
        human = ["CF","CF","CF","CF","RF","RF","PK","PK","INT","INT"]
        ai    = ["CF","CF","CF","RF","RF","RF","PK","CF","INT","INT"]
        ...
        assert abs(result["kappa"] - expected_kappa) < 0.0001, (
            f"Expected Îº â‰ˆ {expected_kappa}, got {result['kappa']}"
        )
    ...
    def test_known_kappa_moderate(self):
        ...
        Worked example:
          ...
          Agreements:
            CF==CF âœ“, CFâ‰ RF âœ—, RF==RF âœ“, RFâ‰ CF âœ—,
            PK==PK âœ“, PKâ‰ INT âœ—, INT==INT âœ“, INTâ‰ RF âœ—
    ...
    def test_known_kappa_below_chance(self):
        """
        When agreement is systematically below chance, Îº is negative.

          n = 4 (one item per category)
          human = ["RF", "PK", "CF", "INT"]
          ai    = ["PK", "CF", "INT", "RF"]   â† shifted by one, zero agreement

          Po = 0/4 = 0.0
          Pe = (1/4)(1/4) Ã— 4 = 0.25
          Îº = (0.0 âˆ’ 0.25) / (1.0 âˆ’ 0.25) = âˆ’0.25 / 0.75 = âˆ’0.3333...
        """
        human = ["RF", "PK", "CF", "INT"]
        ai    = ["PK", "CF", "INT", "RF"]
...
class TestCohenKappaEdgeCases:
    ...
    def test_empty_input(self):
        """Empty lists â†’ kappa=0.0, n=0, no crash."""
        result = compute_cohens_kappa([], [])
        assert result["kappa"] == 0.0
        assert result["n"] == 0

    def test_single_item_agreement(self):
        """Single item, both agree â†’ Îº = 1.0 (pe = 1, guarded case)."""
    ...
    def test_kappa_bounded(self):
        """Îº should never exceed 1.0 for any valid input."""
        import random
        random.seed(42)
        for _ in range(50):
            ...
            result = compute_cohens_kappa(human, ai)
            assert result["kappa"] <= 1.0, (
                f"Îº exceeded 1.0: {result['kappa']} for human={human}, ai={ai}"
            )
...
class TestCohenKappaInterpretation:
    ...
    def _get_interp(self, kappa_val):
        ...
        # For interpretation tests, exact kappa doesn't matter â€” we need
    ...
    def test_above_threshold_contains_check(self):
        """Îº â‰¥ 0.70 interpretation should contain the checkmark."""
        human = ["CF","CF","CF","CF","RF","RF","PK","PK","INT","INT"]
        ai    = ["CF","CF","CF","RF","RF","RF","PK","CF","INT","INT"]
        result = compute_cohens_kappa(human, ai)
        # Îº â‰ˆ 0.7222 â€” should be "Substantial âœ“"
        assert "threshold" in result["interpretation"]

    def test_below_threshold_no_checkmark(self):
        """Îº < 0.70 interpretation should not contain the checkmark."""
        human = ["CF","CF","RF","RF","PK","PK","INT","INT"]
        ai    = ["CF","RF","RF","CF","PK","INT","INT","RF"]
        result = compute_cohens_kappa(human, ai)
        # Îº â‰ˆ 0.3333 â€” should be "Fair"
        assert "âœ“" not in result["interpretation"]


# ===========================================================================
# SECTION 2 â€” build_confusion_matrix()
# ===========================================================================

class TestBuildConfusionMatrix:
    """Tests for the 4Ã—4 confusion matrix constructor."""

    def test_diagonal_on_perfect_agreement(self):
        """All agreements â†’ only diagonal cells are non-zero."""
        human = ["RF", "PK", "CF", "INT"]
        ai    = ["RF", "PK", "CF", "INT"]
        matrix = build_confusion_matrix(human, ai)

        for label in ERROR_TYPES:
            assert matrix[label][label] == 1, f"Diagonal cell {label}Ã—{label} should be 1"
            for other in ERROR_TYPES:
                if other != label:
                    assert matrix[label][other] == 0

    def test_off_diagonal_on_total_disagreement(self):
        """Complete disagreement â†’ only off-diagonal cells are non-zero."""
        human = ["RF", "PK", "CF", "INT"]
        ai    = ["PK", "CF", "INT", "RF"]
        matrix = build_confusion_matrix(human, ai)
        ...
        for label in ERROR_TYPES:
            assert matrix[label][label] == 0, (
                f"Diagonal cell {label}Ã—{label} should be 0 for total disagreement"
            )
    ...
    def test_specific_cell_counts(self):
        ...
        Manually verify cell counts for a known input.
          ...
          Expected matrix:
            ...
            RF         1   0   0    0
            PK         0   0   0    0
            CF         1   0   1    0   â† CFâ†’CF (agree) and CFâ†’RF (disagree)
            INT        0   0   0    0
        """
        human  = ["CF", "CF", "RF"]
        ai     = ["CF", "RF", "RF"]
...
# ===========================================================================
# SECTION 3 â€” get_per_type_stats()
# ===========================================================================
...
class TestGetPerTypeStats:
    ...
    def test_perfect_agreement_all_types(self):
        """All agreed â†’ each type should show 100% agreement."""
        human = ["RF", "PK", "CF", "INT"]
        ai    = ["RF", "PK", "CF", "INT"]
    ...
    def test_zero_agreement_all_types(self):
        """No agreement â†’ 0% for all types present."""
        human = ["RF", "PK", "CF", "INT"]
        ai    = ["PK", "CF", "INT", "RF"]
        stats = get_per_type_stats(human, ai)
...
# ===========================================================================
# SECTION 4 â€” Integration: kappa + matrix + stats are consistent
# ===========================================================================
...
# ===========================================================================
# SECTION 5 â€” Mathematical properties and additional boundary conditions
# ===========================================================================
...
class TestAdditionalProperties:
    ...
    def test_kappa_is_symmetric(self):
        """
        Cohen's Îº is symmetric: Îº(h, ai) == Îº(ai, h).

        Proof sketch:
          Po = agreements/n is unchanged by swapping the two lists.
          Pe = Î£_k p_h_k Ã— p_ai_k = Î£_k p_ai_k Ã— p_h_k  (multiplication is commutative).
          Therefore Îº = (Po âˆ’ Pe)/(1 âˆ’ Pe) is identical in both directions.
        """
        human = ["CF", "CF", "RF", "RF", "PK", "INT", "CF", "RF", "INT", "PK"]
        ai    = ["CF", "RF", "RF", "CF", "PK", "INT", "CF", "RF", "PK",  "PK"]
        ...
        assert abs(kappa_h_ai - kappa_ai_h) < 1e-9, (
            f"Îº not symmetric: Îº(h,ai)={kappa_h_ai:.6f}, Îº(ai,h)={kappa_ai_h:.6f}"
        )
    ...
    def test_pe_guard_all_same_category_both_raters(self):
        ...
        The Pe = 1.0 guard should return Îº = 1.0 because Po = 1.0 too.

        Without the guard, the formula yields 0/0 (division by zero).
        Correct convention: unanimous perfect agreement â†’ Îº = 1.0.
        """
        human = ["RF"] * 8
        ai    = ["RF"] * 8
        result = compute_cohens_kappa(human, ai)
        ...
        assert result["kappa"] == 1.0, (
            f"Pe=1.0 guard failed: got Îº={result['kappa']}"
        )
        assert result["agreements"] == 8

    def test_large_n_does_not_crash_and_stays_bounded(self):
        """
        Îº computation on n = 1 000 items should complete without error
    ...
    def test_per_type_agreed_never_exceeds_total(self):
        """
        For every error type, agreed â‰¤ total.

        Violated only if a rater were credited with an agreement on an item
        they did not actually label â€” a structural impossibility that this
